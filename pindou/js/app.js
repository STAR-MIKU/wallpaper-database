...
const HistoryManager = {
  ...
  set(entry) {
    localStorage.setItem(this._key, JSON.stringify(entry));
  },
...
/**
 * BeadPatternApp — UI logic: upload, crop, render, compare, draft, export.
 */
class BeadPatternApp {
  ...
  _loadImageFromUrl(url, name, source = 'gallery', galleryTemplate = null) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        // Convert to data URL for preview & draft
        const cvs = document.createElement('canvas');
  ...
  _renderHistory() {
    ...
    const entry = HistoryManager.get();
    if (!entry) {
      container.innerHTML = '<p class="history-empty">暂无草稿记录</p>';
      return;
    }
    container.innerHTML = `
      <div class="history-item">
        <img class="history-thumb" src="${entry.thumb}" alt="缩略图">
        <div class="history-meta">
          <div class="h-size">${entry.width}×${entry.height} · ${entry.colors}色 · ${entry.beads}珠</div>
          <div class="h-date">${entry.date} · ${BEAD_PALETTES[entry.brand]?.name || entry.brand}</div>
        </div>
        <button class="history-del" title="清除">✕</button>
      </div>
    `;

    container.querySelector('.history-del').addEventListener('click', e => {
      e.stopPropagation();
